...
\begin{remark}
  ...
  \begin{enumerate}
    \item
      Preadditive categories are also known as~\emph{\dash{$\Ab$}{categories}}\index{Ab-category@$\Ab$-category}\index{category!Ab-@$\Ab$-} (where~$\Ab$ denotes the category of abelian groups).
    \item
...
\begin{definition}
  ...
  \begin{enumerate}
    \item
      If~$\Acat$ and~$\Bcat$ are preadditive categories then the functor~$F$ is \emph{additive}\index{additive!functor}\index{functor!additive} if
      \[
          F(f + g)
        = F(f) + F(g)
      \]
...
\begin{definition}
  A preadditive (or {\preklin}) category~$\Acat$ is \emph{additive}\index{additive!category}\index{category!additive} (respectively~{\klin}\index{k-linear@$\kf$-linear!category}\index{category!k-linear@$\kf$-linear}) if it has finite biproducts (and thus equivalently finite products, and equivalently finite coproducts).
\end{definition}
