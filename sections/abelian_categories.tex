...
%               \end{align*}
    ...
    \item
      ...
      \begin{examplenonum}
        Let~$X \defined \sphere^1$ and let~$\Fs \defined \Gs \defined \Cs^\infty$ be the sheaf of smooth \dash{real}{valued} functions on~$\sphere^1$, which is given by
        \[
            \Cs^\infty(U)
          = \{
              f \colon U \to \Real
    ...
    \item
      ...
      If a presheaf~$\Fs$ is already a sheaf, then it follows from the fully faithfulness of the inclusion~$I$ that the sheafification~$SI(\Fs)$ of~$\Fs$ is just~$\Fs$ itself.
      
      To be more precise, the sheaf~$\Fs$ is also a sheafification of the presheaf~$I(\Fs)$, in the sense that the identity~$\id_{I(\Fs)} \colon I(\Fs) \to I(\Fs)$ fullfills the same universal property as the counit~$\eta_{I(\Fs)} \colon I(\Fs) \to ISI(\Fs)$:
...
%           TODO: How these isomorphisms look like explicitely, i.e. that eta is an iso.
          ...
          \item
            ...
            \end{itemize}
            Proofs of this can be found in~\cite{stackfullyfaithful}, \cite[Lemma 7.6.6]{Brandenburg} and \cite[IV,~3,~Theorem~1]{Working} (where a slightly more general version is proven).
        \end{enumerate}
      \end{remark*}
    ...
    \item
      ...
      \begin{proof}
        Let~$\eta \colon \coker_{\Pcat}(I(f)) \to IS( \coker_{\Pcat}(I(f)) )$ be the canonical homomorphism, and let~$c' \colon I(\Gs) \to \coker(I(f))$ be the morphism belonging to the cokernel~$\coker(I(f))$.
