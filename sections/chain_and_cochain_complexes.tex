  ...
  \begin{enumerate}
    ...
    \item
      ...
      \[
        ...
        \end{tikzcd}
      \]
      The four trapezoids commute by the definitions of~$d$,$d$ and~$f$, and the outer square commutes because~$(f_n)_{n \in \Integer}$ is a morphism of chain complexes.
...
\begin{remark}[Functoriality of homology]
  ...
  \begin{enumerate}
    ...
    It follows from the functoriality of both~$\Bl_n$ and~$\Zl_n$, together with the functoriality of the cokernel (as explained in \cref{functoriality of (co)kernel}), that this induced morphism is both functorial and additive in the following sense:
    \begin{itemize}
      \item
...
\begin{remark*}
  We have seen in~\cref{Hn as cokernel with Cn} that the~\dash{$n$}{th} homology~$\Hl_n(\Ccc)$ of a chain complex~$\Ccc$ can also be described as the cokernel of the morphism~$C_{n+1} \to \Zl_n(\Ccc)$ that is induced by the morphism~$d_{n+1} \colon C_{n+1} \to C_n$ via the universal property of the kernel~$\Zl_n(\Ccc) = \ker(d_n)$ .
...
\begin{proof}
  It suffices to consider the category~$\Ch(\Acat)$.
...
\begin{proof}
  It suffices to show that~$\Ch(\Acat)$ is abelian.
...
\begin{remark*}
  ...
  Kernels and cokernels in the abelian categories~$\Ch(\Acat)$ and~$\CCh(\Acat)$ are computed degree-wise, so the same goes for images and coimages.
  It follows that a sequence~$\Ccc' \to \Ccc \to \Ccc''$ of chain complexes in~$\Acat$ (respectively a sequence~$\bp\Cccc \to \Cccc \to \bpp\Cccc$ of cochain complexes in~$\Acat$) is exact if and only if it is exact in each degree, i.e., if and only if the sequence~$C'_n \to C_n \to C''_n$ (respectively the sequence~$\bp C^n \to C^n \to \bpp C^n$) is exact for every~$n \in \Integer$.
\end{remark*}
