...
\begin{remark*}[Quotient categories]
  ...
  \begin{enumerate}
    \item
      ...
      Then we can try to define a \emph{quotient category}\index{quotient category}\index{category!quotient}~$\widetilde{\Ccat}$, that is supposed to be given
      \begin{itemize}
        \item
      ...
      The collection of equivalence relations~$\sim \defined (\sim_{X,Y})_{X, Y \in \Ob(\Ccat)}$ is then a \emph{congruence relation}\index{congruence relation} on~$\Ccat$, and the quotient category~$\widetilde{\Ccat}$ as described above is denoted by~$\Ccat/{\sim}$.
