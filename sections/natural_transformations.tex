...
\begin{remark}
  ...
  \begin{enumerate}
    \item
      ...
      To see that this is indeed a natural transformation we consider the following diagram:
      \[
        \begin{tikzcd}
            F(X)
        ...
        \end{tikzcd}
      \]
      In this diagram the left and right squares commute because~$\eta$ and~$\zeta$ are natural transformations, and the upper and lower triangles commute by definition of~$\zeta \circ \eta$.
...
\begin{remarkdefinition}
  ...
  If such a natural transformation exists, then it holds for every~$X \in \Ob(\Ccat)$ that
  \[
      \zeta_X \circ \eta_X
    = (\zeta \circ \eta)_X
    = (\id_F)_X
  ...
  \begin{enumerate}
    \item
      ...
      \[
        ...
        \cong X
      \]
      for~$M \in \Ob(\Modl{\kf Q})$ and~$X \in \Ob(\Rep{\kf}{Q})$ are natural, i.e., that for every homomorphism of left~{\modules{$\kf Q$}}~$f \colon M \to N$ the square
      \[
        \begin{tikzcd}[column sep = large]
            FG(M)
            \arrow{r}[above]{FG(f)}
