...
\begin{definition}
  ...
  An \emph{adjunction}\index{adjunction}\index{functor!adjoint} (or \emph{adjoint pair}\index{adjoint pair}) from~$\Ccat$ to~$\Dcat$ is a triple~$(F,G,\varphi)$ consisting of two functors~$F \colon \Ccat \to \Dcat$ and~$G \colon \Dcat \to \Ccat$, together with a family~$(\varphi_{X,Y})_{X \in \Ob(\Ccat), Y \in \Ob(\Dcat)}$ of bijections
  \[
            \varphi_{X,Y}
    \colon  \Dcat(F(X), Y)
    \to     \Ccat(X, G(Y))
