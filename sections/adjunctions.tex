...
\begin{remark*}
  ...
  \begin{enumerate}
    ...
    \item
      \label{right adjoint via representing}
      Dually, a functor~$F \colon \Ccat \to \Dcat$ admits a right adjoint functor~$\Dcat \to \Ccat$ if and only if the functor~$\Dcat(F(-),Y) \colon \Ccat \to \Set$ is representable for every object~$Y \in \Ob(\Dcat)$.
  \end{enumerate}
  (Part~\ref*{left adjoint via representing} is Exercise~1 of Exercise~sheet~4, and part~\ref*{right adjoint via representing} is dual to part~\ref*{left adjoint via representing}.)
\end{remark*}
