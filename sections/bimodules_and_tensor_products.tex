...
\begin{remark}
  ...
  \begin{enumerate}
    \item
      If~$\indmodule[A]{M}[B]$ and~$\indmodule[A]{N}[B]$ are~{\modules{$A$}[$B$]} then a map~$f \colon M \to N$ is a \emph{homomorphism of bimodules}\index{homomorphism!of bimodules}\index{bimodule!homomorphism of} if it is both a homomorphism of left~{\modules{$A$}} and a homomorphism of right~{\modules{$B$}}.
      ...
      The~{\module{$A$}[$B$]} structure of~$M$ then restricts to an~{\module{$A$}[$B$]} module structure on~$N$, which is the unique one that makes the inclusion~$N \to M$ a homomorphism of~{\modules{$A$}[$B$]}.
    \item
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item
      A pair~$(T,\tau)$ consisting of a~{\module{$\kf$}}~$T$ and an~{\balanced{$A$}} map~$\tau \colon M \times N \to T$ is a \emph{tensor product}\index{tensor product!of modules} of~$M$ with~$N$ over~$A$ if it satisfies the following universal property:
...
\begin{warning*}
  ...
  Such elements of~$M \tensor_A N$ are \emph{simple tensors}\index{tensor!simple}\index{simple!tensor}, and every element of~$M \tensor_A N$ is a~{\klin} combination of simple tensors.
  
  It often suffices to check things for simple tensors, since for every~{\module{$\kf$}}~$P$ and every~{\klin} map~$f \colon M \tensor_A N \to P$, the action of~$f$ is already uniquely determined by its action on the simple tensors.
  But to construct such a~{\klin} map one \emph{has to} (in some instance) evoke the universal property of the tensor product;
  it does not suffice to check~\dash{well}{definedness} (or~\dash{$\kf$}{linearity}) on simple tensors.
\end{warning*}
...
\begin{proof}
  ...
  It follows that~$\tau_a$ induces a {\welldef}~{\klin} map
  \[
            M \tensor_B N
    \to     M \tensor_B N \,,
    \quad   x \tensor y
